...
strictMerged=`cat Deliverables/${DIR}.Stats.StandardMerge.Combined.txt | grep "Raw" -A2 | grep "Total" | cut -f2 -d " "`;
standardMerged=`bzcat Deliverables/${DIR}.MergedStandard.fq.bz2 | grep "@HWI-" | wc -l`;
numSamples=`cat Deliverables/${DIR}.Stats.StandardMerge.Combined.txt | cut -f1 -d " " | wc -l`;
