...
usearch70 -usearch_global ${TMPDIR}/uparse{}/otus.fa -db /gpfs1/db/silva/123/silva_V4.udb -id .968 -strand plus -threads ${THREADS} -uc ${TMPDIR}/uparse{}/otus2taxa.uc -maxaccepts 0 -maxrejects 0;
...
cd ${TMPDIR}/uparse{};
${WONGGITREPO}/16S_workflows/resolveIterativeUparse.pl ${TMPDIR}/uparse{}/cluster_*.uc ${TMPDIR}/uparse{}/singletons2otus.uc ${TMPDIR}/uparse{}/otus2taxa.uc --derep ${TMPDIR}/uparse{}/derep.uc --chimeras ${TMPDIR}/uparse{}/chimeras.txt --uchime ${TMPDIR}/uparse{}/uchimeref.uc --taxonomy /gpfs1/db/silva/123/silva.map;
...
perl ${GITREPO}/16S/StatsComparisonMergedVsMapped.pl  ${READSDIR}/../Stats.{}Merge.MergedReads.txt  ${READSDIR}/../Stats.{}Merge.MappedReads.txt ${READSDIR}/../../${PROJECTID}.barcodeCounts.txt > ${READSDIR}/../Stats.{}Merge.Combined.txt;
