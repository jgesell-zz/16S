...
for j in `cat ${input}/samplesheet.*.csv | tail -n +2 | grep -f ${input}/*WorkDir/SampleList`; do barcode=`echo ${j} | cut -f5 -d ","`;
...
else echo ${j} >> ${outputDirectory}/samplesheet.${outname}.csv;
